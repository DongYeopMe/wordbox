...
const WordList = () => {
  ...
  const { cardId, language, apiType, title, cardtitles } = location.state || {
    cardId: null,
    language: "TOTAL",
    apiType: "getList",
    title: "내가 추가한 단어",
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        const endpoint =
          apiType === "getList" ? "/word/getList" : "/word/getMyList"; // API 종류 선택
        const params =
          apiType === "getList"
            ? { language, cardId, page, size: wordPageSize }
            : { language, page, size: wordPageSize };
  ...
  const refreshWordList = async () => {
    try {
      const endpoint =
        apiType === "getList" ? "/word/getList" : "/word/getMyList";
      const params =
        apiType === "getList" ? { language, cardId } : { language };
      const response = await axios.get(`${baseUrl}${endpoint}`, { params });
      setWordList(response.data.data.content);
      setTotalItems(response.data.data.totalElements);
    } catch (error) {
          ...
          wordList.map((word) => (
            <div key={word.id} className="word_space">
              <div className="word" onClick={() => handleWordClick(word)}>
                <span
                  className={selectedView === "hide_word" ? "hide" : "title"}
                >
                  {word.item}
                </span>
                <span
                  className={selectedView === "hide_mean" ? "hide" : "mean"}
                >
                  {word.mean}
                </span>
              </div>
